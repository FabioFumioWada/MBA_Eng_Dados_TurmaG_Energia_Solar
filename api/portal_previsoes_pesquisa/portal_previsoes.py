...
class DatabricksClient:
    ...
    def run_prediction(self, parameters: dict[str, Any]) -> dict[str, Any]:
        ...
        error_trace = output.get("error_trace") or output.get("error")
        if result_state and result_state != "SUCCESS":
            details = error_trace or message or f"state={state}, result={result_state}"
            raise DatabricksError(
                "Notebook terminou sem sucesso "
                f"(run_id={parent_run_id}, task_run_id={child_run_id}):\n{details}"
            )

        result_text = notebook_output.get("result")
        if result_text is None:
            raise DatabricksError(
                error_trace or "Notebook não retornou notebook_output.result"
            )
...
def render_result(response: dict[str, Any]) -> None:
    ...
    if prediction:
        ...
        st.warning("O notebook não retornou a previsão t1.")
